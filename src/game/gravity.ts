...
export const GRAVITY_REACH_FACTOR = 2.1;

/** Per-planet cap so a few planets can't hoard the whole sky. */
export const MAX_CAPTURED_PER_PLANET = 10;
...
function tryCapture(d: Dust, planets: Planet[], population: Dust[]): void {
  for (let i = 0; i < planets.length; i++) {
    ...
    const orbitRadius = Math.min(Math.max(dist, p.radius + 6), p.radius * 1.9);
    const direction = Math.random() < 0.5 ? 1 : -1;
    d.capturedBy = i;
    d.orbitRadius = orbitRadius;
    d.orbitAngle = Math.atan2(d.y - p.y, d.x - p.x);
    d.orbitSpeed = (0.5 + 26 / orbitRadius) * direction;
    d.releaseIn = 5 + Math.random() * 7;
    return;
  }
}
